...
The fireStrat package allows you to automatically partition the environmental space of a high severity fire. It partitions the following environmental variables: elevation. slope, and winter solstice insolation. A specified number of partitions is set by the user. Representative points within the partitions are identified to assist with sampling guidance. 
...
Next the user calls the fireStrat() function, specifying the above layers, the desired number of zones (minimum of 5 maximum of 18), number of backup points within each of those zones, and mean December temperature for the region (dec.t argument). The latter is needed for the production of the winter solstice insolation model. 
...
**stratified.espace.csv** - a table of all map grid cells containing coordinates, environmental values, and zone assignments
